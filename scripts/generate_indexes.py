...
def generate_index_for_dir(
    dir_path: Path, is_root: bool = False
    ...
    digit_children = sorted(
        [
            ...
            if (
                is_digit_prefixed(item.name)
                or (is_root and item.suffix == ".md" and item.name != "index.md")
            )
    ...
    if (
        not is_root
        and len(digit_children) == 1
        and not (dir_path / ".header.md").exists()
    ):
