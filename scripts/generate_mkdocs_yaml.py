    ...
    for child in digit_children:
        if child.is_file() and child.suffix == ".md":
            ...
            if isinstance(sub_res, dict):
                ...
                title = (
                    get_first_heading(child_index)
                    if child_index.exists()
                    else child.name.split("-", 1)[-1].replace("-", " ").title()
                )
                if child_index.exists():
                    items.append(
                        {title: [str(child_index.relative_to(DOCS_DIR))] + sub_res}
                    )
                else:
                ...
                items.append(
                    {
                        get_first_heading(child_index): str(
                            child_index.relative_to(DOCS_DIR)
                        )
                    }
                )
